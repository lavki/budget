        <div class="col-md-8 col-md-offset-2">
            ...
            <form action="<?= base_url('/category/create'); ?>" method="post" class="row" autocomplete="off">
                <div class="form-group col-md-4 col-md-offset-2 <?= empty(form_error('category' )) ? '' : ' has-error'; ?>">
                    ...
                    <input type="text" name="category" id="category" minlength="3" maxlength="50" value="<?= set_value('category', $category ?? null ); ?>" class="form-control input-sm" required aria-describedby="helpBlock">
                    <?= form_error('category' ); ?>
                </div><!-- /.form-group -->
                <div class="form-group col-md-4 <?= empty(form_error('type' )) ? '' : ' has-error'; ?>">
                    ...
                    <select name="type" id="type" value="<?= set_value('type', $type ?? null ); ?>" class="form-control input-sm" required aria-describedby="helpBlock">
                        ...
                        <?php foreach( $types as $type ): ?>
                            <option value="<?= $type->id; ?>" <?= (set_value('type') == $type->id) ? 'selected' : null; ?>><?= $type->status; ?></option>
                        <?php endforeach; ?>
                    </select>
                    <?= form_error('type' ); ?>
                </div><!-- /.form-group -->
